...
CombatScreenHard::CombatScreenHard(SDL_Renderer* renderer, TTF_Font* font)
    ...
    surface = IMG_Load("Assets/Asta.png");
    enemyTexture = SDL_CreateTextureFromSurface(renderer, surface);
    SDL_FreeSurface(surface);
    enemyRect = { 450, 150, 248, 303 };

    surface = IMG_Load("Assets/dmgAsta.png");
    dmgEnemyTexture = SDL_CreateTextureFromSurface(renderer, surface);
    SDL_FreeSurface(surface);
    dmgEnemyRect = { 450, 150, 248, 303 };

    surface = IMG_Load("Assets/SasaInCage.png");
    sasaTexture = SDL_CreateTextureFromSurface(renderer, surface);
    SDL_FreeSurface(surface);
    sasaRect = { 310, 0, 120, 194 };
    ...
    bgMusic = Mix_LoadMUS("Assets/bossMusic.mp3");
    Mix_PlayMusic(bgMusic, -1);
}

CombatScreenHard::~CombatScreenHard() {
    ...
    SDL_DestroyTexture(sasaTexture);

}

void CombatScreenHard::handleEvents(SDL_Event& event) {
...
int CombatScreenHard::getPlayerMove(SDL_Event& event)
{
    if (event.type == SDL_KEYDOWN) {
...
void CombatScreenHard::render(SDL_Renderer* renderer) {
    SDL_Color white = { 255, 255, 255 };
    SDL_RenderClear(renderer);
    if (!player.isDead() && !enemy.isDead())
    {
        renderText("Player HP: " + std::to_string(player.getHP()), 10, 10, white);
        SDL_RenderCopy(renderer, sasaTexture, nullptr, &sasaRect);
        SDL_RenderCopy(renderer, playerTexture, nullptr, &playerRect);
        SDL_RenderCopy(renderer, enemyTexture, nullptr, &enemyRect);
...
bool CombatScreenHard::isFinished() const {
    return finished;
}

bool CombatScreenHard::isSuccessful() const
{
    return successful;
}

void CombatScreenHard::renderText(const std::string& message, int x, int y, SDL_Color color) {
