...
TransitionScreen2::TransitionScreen2(SDL_Renderer* renderer, TTF_Font* font)
    : renderer(renderer), font(font)
{
    SDL_Surface* transSurface = IMG_Load("Assets/combatTransition2.png");
    if (!transSurface) {
...
TransitionScreen2::~TransitionScreen2() {}

void TransitionScreen2::handleEvents(SDL_Event& event) {
    if (event.type == SDL_KEYDOWN) {
        readyToStart = true;
    }
}

void TransitionScreen2::update() {

}


void TransitionScreen2::render(SDL_Renderer* renderer) {
...
bool TransitionScreen2::isReadyToStart() const {
    return readyToStart;
}

void TransitionScreen2::renderText(SDL_Renderer* renderer, const std::string& message, int x, int y, SDL_Color color) {
    if (!font) return;
